...
// long double polish_transformation(struct node * line, int * flag){
        ...
        do {
            switch (line->type) {
                case 0:
                    ...
                    } else if (strchr("~sctYqL+-*/", line->value.symbol)) {
...
struct node* use_unary_operatop(struct node* stack, char operator, int * flag) {
    switch (operator) {
        ...
        case 's':
            stack->value.number = sin(stack->value.number);
            break;

        case 'c':
            stack->value.number = cos(stack->value.number);
            break;

        case 't':
            stack->value.number = tan(stack->value.number);
            break;

        case 'T':
            stack->value.number = 1 / tan(stack->value.number);
            break;

        case 'q':
            stack->value.number = sqrt(stack->value.number);
            break;

        case 'L':
            stack->value.number = log(stack->value.number);
            break;
        default:
            *flag = 0;
            break;
